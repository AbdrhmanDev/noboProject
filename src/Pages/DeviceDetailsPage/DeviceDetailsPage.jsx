...
import { ArrowRight, HardDrive, Pencil, Power, RefreshCw, Unplug } from "lucide-react";
...
import {
  ...
  useUnbindDeviceHardware,
  useUpdateDeviceStatus,
} from "../../features/devices/hooks/useDevices";
import { edgeAgentQueryKeys, useEdgeAgentDetails } from "../../features/devices/hooks/useEdgeAgents";
...
import {
  ...
  DeviceStatusBadge,
  CertificationBadge,
} from "../../features/devices/components/DeviceStatusBadge";
...
export default function DeviceDetailsPage() {
  ...
  const viewPermissionQuery = useHasPermission(
    currentCompanyId,
    DEVICES_VIEW_PERMISSION,
  );
  const managePermissionQuery = useHasPermission(
    currentCompanyId,
    DEVICES_MANAGE_PERMISSION,
  );
  const canQuery =
  ...
  const deviceQuery = useDeviceDetails(
    currentCompanyId,
    currentBranchId,
    deviceId,
    canQuery,
  ...
  // Reused, not duplicated: the exact same lists DeviceFormDialog already fetches for its own
  // selects -- used here only to resolve a name for device.posTerminalId/kitchenStationId instead
  // of showing the raw GUID.
  ...
  const posTerminal = device?.posTerminalId
    ? (posTerminalsQuery.data || []).find((terminal) => terminal.posTerminalId === device.posTerminalId)
    : null;
  const kitchenStation = device?.kitchenStationId
    ? (kitchenStationsQuery.data || []).find((station) => station.kitchenStationId === device.kitchenStationId)
    : null;
  ...
  const unbindMutation = useUnbindDeviceHardware(currentCompanyId, currentBranchId, deviceId);
  const confirmUnbind = async () => {
    try {
      await unbindMutation.mutateAsync();
      toast.success(t("devices.toast.hardwareUnbound"));
    } catch {
      // The mutation's own error stays visible via its isError/error state if a dedicated
      // surface is added later; for now the toast simply doesn't fire on failure.
    } finally {
  ...
  const refreshStatus = () => {
    if (!currentCompanyId || !currentBranchId || !deviceId) return;
    queryClient.invalidateQueries({ queryKey: deviceQueryKeys.details(currentCompanyId, currentBranchId, deviceId) });
    queryClient.invalidateQueries({ queryKey: deviceQueryKeys.hardwareBinding(currentCompanyId, currentBranchId, deviceId) });
    if (device?.edgeAgentId) {
      queryClient.invalidateQueries({
        queryKey: edgeAgentQueryKeys.details(currentCompanyId, currentBranchId, device.edgeAgentId),
      });
    }
    toast.success(t("devices.toast.statusRefreshed"));
  };
      ...
      <main className="space-y-4" dir="rtl">
            ...
            <div className="space-y-4">
              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 text-sm font-black text-white">
                  {t("devices.details.identity")}
                </div>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <InfoTile
                    label={t("devices.device.code")}
                    value={device.code}
                  />
                  <InfoTile
                  ...
                  <InfoTile
                    ...
                    value={t(
                      `devices.enum.deviceType.${lowerFirst(device.deviceType)}`,
                    )}
                  />
                  <InfoTile
                    label={t("devices.device.manufacturer")}
                    value={device.manufacturer}
                  />
                  <InfoTile
                    label={t("devices.device.model")}
                    value={device.model}
                  />
                  <InfoTile
                    label={t("devices.device.serialNumber")}
                    value={device.serialNumber}
                  />
                  <InfoTile
                    label={t("devices.device.firmwareVersion")}
                    value={device.firmwareVersion}
                  />
                  <InfoTile
                    label={t("devices.details.created")}
                    value={formatDateTime(device.createdAtUtc)}
                  />
                  <InfoTile
                    label={t("devices.details.updated")}
                    value={formatDateTime(device.updatedAtUtc)}
                  />
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 text-sm font-black text-white">
                  {t("devices.details.assignment")}
                </div>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <InfoTile
                    label={t("devices.device.posTerminal")}
                    value={
                      !device.posTerminalId
                        ? t("devices.device.none")
                        : posTerminal
                          ? `${posTerminal.code} — ${posTerminal.name}`
                          : device.posTerminalId
                    }
                  />
                  <InfoTile
                    ...
                    value={
                      !device.kitchenStationId
                        ? t("devices.device.none")
                        : kitchenStation
                          ? `${kitchenStation.code} — ${kitchenStation.name}`
                          : device.kitchenStationId
                    }
                  />
                  <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
                    ...
                    {device.edgeAgentId ? (
                      <button
                        type="button"
                        onClick={() =>
                          navigate(edgeAgentDetailsPath(device.edgeAgentId))
                        }
              ...
              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-black text-white">
                    {t("devices.details.hardwareBinding")}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={refreshStatus}
                      className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.035] px-2.5 py-1.5 text-[11px] font-bold text-slate-100 hover:bg-white/10"
                    >
                      <RefreshCw size={12} />
                      {t("devices.details.refreshStatus")}
                    </button>
                    {canManage && device.edgeAgentId && (
                      <button
                        type="button"
                        onClick={() => setShowRebindDialog(true)}
                    ...
                    {canManage && hardwareBindingQuery.data && (
                      <button
                        type="button"
                        onClick={() => setShowUnbindConfirm(true)}
                        className="flex items-center gap-1.5 rounded-lg border border-rose-400/25 bg-rose-500/10 px-2.5 py-1.5 text-[11px] font-bold text-rose-200 hover:bg-rose-500/20"
                      >
                        <Unplug size={12} />
                        {t("devices.unbind.action")}
                      </button>
                    )}
                  </div>
                </div>
                {hardwareBindingQuery.isLoading ? (
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    ...
                    {hardwareBindingQuery.data.networkAddress && (
                      <InfoTile
                        ...
                        value={`${hardwareBindingQuery.data.networkAddress}${
                          hardwareBindingQuery.data.networkPort
                            ? `:${hardwareBindingQuery.data.networkPort}`
                            : ""
                        }`}
                      />
                    ...
                    {hardwareBindingQuery.data.comPort && (
                      <InfoTile
                        label={t("devices.discovery.comPort")}
                        value={hardwareBindingQuery.data.comPort}
                      />
                    )}
                    ...
                    <InfoTile
                      ...
                      value={formatDateTime(
                        hardwareBindingQuery.data.confirmedAtUtc,
                      )}
                    />
                  </div>
                ) : (
              ...
              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 text-sm font-black text-white">
                  {t("devices.details.connectivity")}
                </div>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <InfoTile
                    ...
                    value={
                      device.connectionType
                        ? t(
                            `devices.enum.connectionType.${lowerFirst(device.connectionType)}`,
                          )
                        : "—"
                    }
                  />
                  <InfoTile
                    label={t("devices.details.lastSeen")}
                    value={
                      device.lastSeenAtUtc
                        ? formatDateTime(device.lastSeenAtUtc)
                        : "—"
                    }
                  />
                  <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
              ...
              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 text-sm font-black text-white">
                  {t("devices.details.certification")}
                </div>
                <CertificationBadge
                  certification={device.certificationStatus}
                />
              </section>

              <section className="rounded-2xl border border-white/10 bg-[#0c1424] p-4">
                <div className="mb-3 text-sm font-black text-white">
                  {t("devices.details.printHistory")}
                </div>
        ...
        {showUnbindConfirm && device && (
          <DeviceConfirmActionDialog
            title={t("devices.unbind.action")}
            message={t("devices.unbind.confirmMessage", {
              device: device.name,
              assignment: kitchenStation?.name || posTerminal?.name || t("devices.device.none"),
            })}
            confirmLabel={t("devices.unbind.action")}
            tone="danger"
        ...
        {showRebindDialog && device && currentCompanyId && currentBranchId && device.edgeAgentId && (
          <RebindDeviceHardwareDialog
            companyId={currentCompanyId}
            branchId={currentBranchId}
            device={device}
            currentBinding={hardwareBindingQuery.data}
            edgeAgentId={device.edgeAgentId}
            onClose={() => setShowRebindDialog(false)}
            onSuccess={() => setShowRebindDialog(false)}
          />
        )}
